/*******************************************************************************
   ...
   ====== =========== ===== ==============================================
          07/24/2019  jiaqi  Initial Draft
...
#define DPS_LOG_FILE_VERSION1       (1)
   ...
   */
   ...
   public:
      ...
      // read from file
      INT32 read ( const UINT32 &lOffset, UINT32 len, CHAR *buf ) ;
      // close the file
      INT32 close();
      ...
      // reset metadata
      INT32 reset ( UINT32 logID, const UINT64 &offset ) ;
      ...
      BOOLEAN isZeroStart()
      {
         if ( _logHeader._logID != DPS_INVALID_LOG_FILE_ID &&
              _logHeader._offset % _fileSize == 0 )
         {
            return TRUE ;
         }
         return FALSE ;
