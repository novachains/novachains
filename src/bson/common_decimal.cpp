                      ...
                      bson_decimal *result )
{
   ...
   res_buf = (short*)malloc( ( res_ndigits + 1 ) * sizeof( short ) ) ;
   if ( NULL == res_buf )
   {
      rc = -2 ;
      goto error ;
                  ...
                  bson_decimal *result, int rscale, int isRound )
{
    ...
    */
   {
      ...
      dividend = ( short * )malloc( tmpDividendLen * sizeof( short ) ) ;
      if ( NULL == dividend )
      {
         rc = -2 ;
         goto error ;
...
int _decimal_alloc( bson_decimal *decimal, int ndigits )
{
   ...
   decimal->buff = (short*)malloc( (ndigits + 1) * sizeof(short) ) ;
...
// the caller is responsible for freeing this value(free)
...
#ifdef _WINDOWS
   _sprintf( buf, "%.*g", SDB_DECIMAL_DBL_DIG, value ) ;
#else
   sprintf( buf, "%.*g", SDB_DECIMAL_DBL_DIG, value ) ;
#endif //_WINDOWS
...

{
   ...
   */
   ...
   decdigits = (unsigned char *) malloc( strlen(cp) +
...
int decimal_from_bsonvalue( const char *value, bson_decimal *decimal )
{
   ...
   {
      decimal->digits[index] = *(short*)value ;
      value += sizeof( short ) ;
   }
...
int decimal_copy( const bson_decimal *source, bson_decimal *target )
{
   ...
   newbuf    = (short*)malloc( (source->ndigits + 1) * sizeof(short) );
   if ( NULL == newbuf )
   {
      rc = -2 ;
      goto error ;
